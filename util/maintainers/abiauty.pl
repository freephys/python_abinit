...
# Copyright (C) 2007-2009 ABINIT group (LSi)
...
@UpCaseExcl =('DOS','DOWN','DOUBLE','DOCTYPE','MPI_SEND','ENDDEF');
...
$IndentCols = 1;	# columns count for normal indentation
...
# b1) prepare to remove blanks starting at column 2, for comments or continuations too, or at column 1 for cpp directives
			$line5 = $line;
			$ix = 0;
			while ($ix < $len) {
...
# b2) align comments and continuations to column 1
			elsif (($charx eq '&' || $charx eq '!') && $noabicnt == 0) {
				$line = substr($line,$ix);
				$char1 = $charx;
				$len -= $ix;
				$ix = 1;
...
# beginning and end of this construct and will probably find an inconsistancy later in the source file.
...
# help the programmer find his error, a simple warning is displayed in such a case.
			...
			if ($char1 ne '!') {
				...
				print "Warning: upper case construct keyword(s) $doifcase"."found at line $linect" if ($doifcase ne '');
				}
...
# This is prohibited by coding rules! See note at step a) above
			...
			while (1) {
				...
				$kwd = 'then' if ($ix2 > 0);
				if ($wd1 eq 'do' || (substr($wd1wd2,0,3) eq 'if(' && $ix2 > 0) || $wd1 eq 'else' || substr($wd1wd2,0,7) eq 'elseif(' || ($wd1 eq 'select' && substr($wd2wd3,0,5) eq 'case(' ) || substr($wd1wd2,0,5) eq 'case(' || ($wd1 eq 'end' && ($wd2 eq 'do' || $wd2 eq 'if' || $wd2 eq 'select') ) ) {
					$doifcase = "$doifcase$kwd ";
					print "Error: $kwd construct statement encountered in no_abiauty section at line $linect" if ($noabicnt > 0); 
					}
				if ($ix < 1) {	# semicolon not found or first char
					print "Warning: semicolon found at line $linect with $doifcase statement" if ($semicols > 0 && $doifcase ne '');
					last;				# exit loop
					}
...
# turn flag on if Robodoc last line was found
			if ($EndRobodoc == 0 && $wd1 eq $RobodocLast) {
				...
				print 'ERROR, executable section not found' if ($phase == 4);
				$EndRobodoc = 1;
			...
			if ($wd1 ne '' && $char1 ne '!' && $char1 ne '#') { 
				if ($wd1 eq 'program' || $wd1 eq 'module' || $wd1 eq 'interface' || $wd1 eq 'subroutine' || $wd1 eq 'function') {
			 	 ...
			 	 &HndlProg($wd3,$wd4);
					...
					$ProgLvl --;
					if ($ProgLvl < 0) {
						print "Error: found end $wd2 statement at line $linect unrelated to any open block";
						last;
						}
...
# Phase 4: find executable subsection in source
		if ($phase == 4) {
			if ($wd1 eq 'end' && ($wd2 eq 'subroutine' || $wd2 eq 'function' || $wd2 eq 'module')) {
				print "ERROR: found end statement at line $linect for '' $wd2 $wd3 '' before executable section";
				&WriteLine($line,$len);
				$itemNum = 0;
				$phase = 3;
				next;
...
# a) handle cpp #if-#endif blocks
			if (substr($wd1,0,3) eq '#if') {
				...
				if ((substr($wd1wd2,3,3) eq 'def' || index($line,' defined ') > 0) && index($line,' MPI') > 0) {
					...
					print "Error, MPI definition checked in no_abiauty section at line $linect" if ($noabicnt > 0);
					}
				}
			elsif ($wd1wd2 eq '#endif') {
				if ($cppifLvl <= 0) {
					print "ERROR: found #endif cpp directive at line $linect unrelated to any #if open block";
					last;
					}
				print "#endif closes cpp #if block at line $linect" if ($debug > 1);
				$cppifLvl --;
				if ($cppifLvl < $MPIdefLvl) {
					print "Error, end of MPI block encountered in no_abiauty section at line $linect" if ($noabicnt > 0);
					$MPIdefLvl = 0;
					}
				}
...
# f2) handle else, 'elseif (' or 'elseif('
			if ($wd1 eq 'else' || substr($wd1wd2,0,7) eq 'elseif(') {
			  $indentafter = 1;		# same block-level as previous if and same alignment
				}
...
# g2) handle 'case (' or 'case('
			if (substr($wd1wd2,0,5) eq 'case(' ) {
				if ($ProgLvl > 0) {
					$PrevLvl = $ProgLvl - 1;
					if ($ProgType[$PrevLvl] eq 'select') {
			  		&HndlProg('case','construct');
						...
						print "ERROR: found $wd1wd2 statement at line $linect unrelated to any select block";
						last;
						}
					}
			  $indentafter = 1;
...
# h) check end statements
			if ($wd1 eq 'end' && ($wd2 eq 'subroutine' || $wd2 eq 'module' || $wd2 eq 'interface' || $wd2 eq 'function' || $wd2 eq 'do' || $wd2 eq 'if' || $wd2 eq 'select' || $wd2 eq '') ) {
				...
				if ($ProgType[$ProgLvl] ne $wd2 || ($ProgName[$ProgLvl] ne $wd3 && (($wd3 ne '' && substr($wd3,0,1) ne '!') || $ProgName[$ProgLvl] ne 'construct') ) ) {
					print "ERROR: found end statement at line $linect for '' $wd2 $wd3 '' instead of '' $ProgType[$ProgLvl] $ProgName[$ProgLvl] '' ";
					last;
					}
...
# were all sections and statements encountered ?
  if ($phase < 5 && $phase > 1) {
		print "ERROR: end of file $fname hit prematurely while expecting:";
...
# Common variables: $ProgLvl,$indentafter,$fname,$diffcnt,$noabicnt
	$char1 = substr($line,0,1);
# print cpp directives and empty lines unchanged; set indentation for others
	if ($phase == 5 && $char1 ne '#' && $line ne "\n" && $noabicnt == 0) { 
		$repeatct = ($ProgLvl - $indentafter) * $IndentCols;	# indentation increases AFTER do, if
